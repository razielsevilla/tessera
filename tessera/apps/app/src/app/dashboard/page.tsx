...
const TextArea = ({ label, placeholder, rows = 4, value, onChange }: any) => (
  <div className="flex flex-col gap-1 mb-5">
    <label className="text-xs font-bold tracking-widest uppercase text-stone-400">{label}</label>
    <textarea 
      placeholder={placeholder}
...
const PageHeader = ({ title, description, icon: Icon, colorClass }: any) => {
    ...
    <div className="animation-fade-in">
      ...
      <Card>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <div>
            <Input label="Narrative Title" placeholder="e.g. The Quantum Thief" value={title} onChange={(e: any) => setTitle(e.target.value)} />
            <Input label="Current Chapter / Node" placeholder="e.g. Node 4B - The Heist" value={chapter} onChange={(e: any) => setChapter(e.target.value)} />
            <TextArea label="Decisions & Outcomes" placeholder="Player chose stealth. Companion trust +10..." rows={4} value={decisions} onChange={(e: any) => setDecisions(e.target.value)} />
    ...
    <div className="animation-fade-in">
      ...
      <Card className="relative overflow-hidden bg-[#FCFAF5]">
        ...
        <div className="relative z-10">
          <Input label="Entry Title / Date" placeholder="e.g. Autumn Reflections" value={title} onChange={(e: any) => setTitle(e.target.value)} />
          <TextArea 
            label="Journal Entry" 
...
const MosaicCanvasUi = () => {
  const [mosaicData, setMosaicData] = useState<any[]>([]);
  
  useEffect(() => {
    setMosaicData(Array.from({ length: 144 }).map((_, i) => ({
      id: i,
  ...
  const getColor = (type: string, intensity: number) => {
    ...
    switch(type) {
      case 'life': return `rgba(114, 92, 58, ${opacity})`;
      case 'media': return `rgba(74, 100, 114, ${opacity})`;
      case 'if': return `rgba(107, 76, 90, ${opacity})`;
      case 'retro': return `rgba(74, 93, 78, ${opacity})`;
